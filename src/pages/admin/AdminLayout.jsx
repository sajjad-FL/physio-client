...
export default function AdminLayout() {
  ...
  const navItems = useMemo(
    () =>
      baseNavItems.map(({ countKey, countKeys, ...rest }) => {
        let badgeCount = 0
        if (countKeys?.length) {
          badgeCount = countKeys.reduce((sum, key) => sum + (Number(counts[key]) || 0), 0)
        } else if (countKey != null) {
          badgeCount = Number(counts[countKey]) || 0
        }
        return { ...rest, badgeCount }
      }),
    [counts],
  )
  ...
  return (
    <>
      ...
      <AppShell
        ...
        topBarSubtitle="Bookings, payments, people & compliance"
        navItems={navItems}
        headerActions={
          <Link
            to="/"
        ...
        sidebarFooter={
          <p className="text-center text-xs leading-relaxed text-slate-400">
            Profile &amp; sign out are in the menu above.
          </p>
        }
      >
        <Outlet />
